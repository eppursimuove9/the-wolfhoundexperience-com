...
abstract class Config extends Cache
{
    ...
    {
        return get_option( 'bookly_tag_colors', array( '#17a2b8', '#6c757d', '#28a745', '#dc3545', '#ffc107', '#007bff', '#343a40' ) );
    }

    /**
     * Get a tag list.
     * @param string $type services, customers
     *
     * @return array
     */
    public static function getTagList( $type = Entities\Tag::TYPE_SERVICE )
    {
        $tags = Entities\Tag::query()
            ->select( 'id, tag, color_id, attachment_id, info' )
            ->where( 'type', $type )
            ->fetchArray();

        foreach ( $tags as &$tag ) {
            $tag['attachment'] = BooklyLib\Utils\Common::getAttachmentUrl( $tag['attachment_id'], 'full' ) ?: null;
        }
